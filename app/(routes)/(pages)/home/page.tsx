...
const Home: React.FC = () => {
  ...
  // {(formato data)(data), (in minuti)(orario_fine), (in minuti)(orario_inizio)}
  // needed for API calls
  const timeFrame: RefObject<TimeFrame> = useRef({data: new Date(), inizio: 0, fine: 0})
        ...
        <div id="left-bar" className="grid grid-rows-[2fr_7fr] border-solid ">
          ...
          <div id="date-time" className="w-full grid-rows-[4fr_6fr]">
            <div id="hour-selector" className="grid justify-between w-full aspect-[3] grid-cols-[80%_20%] border-solid border-[0px] border-black">
              ...
              <HourDrawer timeFrameRef={timeFrame} id="hour-drawer" inizioTextRef={oraInizio} fineTextRef={oraFine} className="grid grid-rows-[1fr_1fr_1fr] grid-cols-[1fr_1fr]" />
            </div>
            <DatePicker timeFrameRef={timeFrame} id="date-picker" className="" />
          </div>
        </div>
