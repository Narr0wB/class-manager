...
const QUERY_SELECT_PRE_ALL_EMAIL = "SELECT * FROM AM_Prenotazioni WHERE email_utente = ?";
const QUERY_SELECT_PRE_ALL_DATARANGE = "SELECT * FROM AM_Prenotazioni WHERE data_ora_pre BETWEEN ? and ?"
...
const QUERY_SELECT_UTENTE_ALL_EMAIL = "SELECT * FROM AM_Utenti WHERE email_utente = ?";
const QUERY_SELECT_AULA_ALL = "SELECT * FROM AM_Aule";
...
export async function SELECTPrenotazioniDate(date_start: Date, date_end: Date, aula?: Aula) {
  ...
  const ret = await query<Prenotazione>(
    QUERY_SELECT_PRE_ALL_DATARANGE,
